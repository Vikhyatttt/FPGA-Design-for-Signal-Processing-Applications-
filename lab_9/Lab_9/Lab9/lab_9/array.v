...
generate
	for (gr=0; gr<rows; gr=gr+1) begin : gencontr	//Input
	...
	for (gr=0; gr<rows; gr=gr+1) begin : genweir	// weight
		for(gc=0;gc<cols;gc=gc+1)begin : genweic
			// start of your code====================================
			// Example:
			 	if(gc==0)begin
			 		assign wire_i_w[gr][gc]=ws[(gc+1)*width-1:(gc)*width];
			 	end
			 	else begin
	...
	for (gr=0; gr<rows; gr=gr+1) begin : genoutr  // Output
		for(gc=0;gc<cols;gc=gc+1)begin : genoutc
			// start of your code====================================
			// Example:
			 	if(gr== (rows - 1))begin
